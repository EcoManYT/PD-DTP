...
Also if have any issues dm me on reddit username: Eco_the_redditor
...
void loop() {
  ...
  for(uint8_t i = 0;i <= 100;i++){          //repeat the below code 100 times
    ...
    if(ballx > dw - 2){ballvelx *= -ballbounciness;}
    // /\
    //when the ball hits the left wall then: reverse the x velocity and multiply it by the ballbounciness
    disp.clearBuffer();                       //clear the memory buffer
